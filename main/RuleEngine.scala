...
object RuleEngine extends App {
  ...
  // 5. Sales made through the App with specified quantity range Qualifying Rule
  // The maximum quantity in the dataset is 17, So I supposed the values of quantities will not exceed 20
  def quantityQualifier(quantity: Int) : Double = {
  ...
  def salesApp(quantity: Int, channel: String): Double = {
    if (channel == "App") quantityQualifier(quantity)
    else 0.0
  }
  ...
  // 6. Payment method Qualifying Rule
  def visaPaymentMethod(paymentMethod: String): Double = {
    if (paymentMethod == "Visa") 0.05
    else 0.0
  }
  ...
  val resultsList: List[List[Double]] = lines.map { line =>
    ...
    logEvent("INFO", "RuleEngine started checking the Channel qualification ...")
    logEvent("INFO", "RuleEngine started checking the Channel calculation.")
    val appDiscount = salesApp(quantity.toInt, channel)
    logEvent("INFO", "RuleEngine completed checking the Channel calculation.")

    logEvent("INFO", "RuleEngine started checking the Payment Method qualification ...")
    logEvent("INFO", "RuleEngine started checking the Payment Method calculation.")
    val visaPayMethod = visaPaymentMethod(payment_method)
    logEvent("INFO", "RuleEngine completed checking the Payment Method calculation.")

    List(daysRemainingDiscount, onSaleDiscount, marchDiscount, quantitySoldDiscountValue, appDiscount, visaPayMethod)
  }
